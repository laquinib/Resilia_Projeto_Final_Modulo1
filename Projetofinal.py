def main():
    ...
    print('\nHoje vamos investigar alguns dos crimes que mais impressionaram '
      'os moradores de Trashville nos últimos meses...\n')
...
def encerra_jogo():
    ...
    while escolha == '1':
        ...
        print('Foi bom acompanhar você nessa missão...')
        print('Até logo!!!!')


def narrativa_Marie():
    opcaoMarie = input('Você está de plantão na delegacia e foi escalada para investigar o '
                       '\ndesaparecimento de uma adolescente. Há suspeitas de que ela poderia ter sido assassinada pelo ex namorado..\n'
                       '\nVocê, agora:\n '

                       '\n[1] Vai ao clube onde ela foi vista pela última vez e registra tudo o que encontrou no local\n'
                       '\n[2] Acompanha a força-tarefa da polícia na missão de procurar o ex namorado da jovem\n')
...
def clube():
    opcao_clube = input(
        'Chegando no clube, você descobre mais informações e então coleta as digitais presentes no banheiro '
        ...
        '\n[2]  Que há dois suspeitos principais até o momento: o ex namorado e a melhor amiga da garota.\n')
...
def enredo1_clube():
    opcao_enredo = input('Após analisar o material recolhido e novas descobertas acontecerem, você foi perseguida'
                         ...
                         '\n[2] As provas indicam a relação de uma terceira pessoa no crime...\n')
    ...
    if opcao_enredo == '1':
        ...
        print('Parece que alguns colegas de trabalho estão desconfiando de você... Fuja enquanto é tempo!!! Fim de jogo!')
    else:
        print('Deseja jogar novamente? ')
        encerra_jogo()

def investigacao_venceu():
...
def narrativa_Cristian():
    crime_escola = int(
        input('Na manhã da última segunda-feira, um assassinato misterioso aconteceu na Escola Estadual Pablo Iglesias.\n '
...
def conclui_crime_escola():
    crime_escola = input(
        ...
        '\n[3] Bento\n ')
    if crime_escola == '1':
        print('Roni é o assassino! Era o primeiro dia de aula e não tinha como avaliar os alunos.')
        ...
        print(
            'Bento trabalhava à tarde e o crime aconteceu pela manhã. GAME OVER! \n')
